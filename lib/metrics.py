...
def calculate_bleu(cap1,cap2):
    logging.basicConfig(level=logging.INFO)
    logger = logging.getLogger('Computing Metrics:')
    args = parser.parse_args()
    references = [['f8k_val_0.txt'],['f8k_val_1.txt'],['f8k_val_2.txt'],['f8k_val_3.txt'],['f8k_val_4.txt']]
    hypothesis = ['use_name']
    #ref, hypo = load_textfiles(references, hypothesis)
    print (score(cap1,cap2))
